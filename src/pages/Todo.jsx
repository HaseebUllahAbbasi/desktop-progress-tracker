...
const Todo = () => {
  ...
  const fetchTodoUpdates = async () => {
    try {
      if (user) {
        const response = await axios.get(SERVER + "/api/todo/" + user?._id);
  ...
  return (
    <div className="container-custom">
      ...
      <div className="todo-input">
        ...
        />
        <button onClick={handleAddTodo} className="btn btn-warning">
          Add
        </button>
      </div>
      <div>
          ...
          .map((todo) => (
            ...
            >
              <AiFillDelete
                style={{ fontSize: "20px" }}
                onClick={() => {
                  const newTodo = todoUpdates.filter(
        ...
        {todoUpdates
          ...
          .map((todo) => (
            <div
              ...
              className={`${
                todo.animate
                  ? "animate shadow-lg  my-2 todo-item"
                  : "shadow-lg my-2"
              }`}
              style={{
                borderRadius: "10px",
